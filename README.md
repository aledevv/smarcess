...
- Pi Camera Module (ov5647 used)
- LCD display (optional)
- Holding structure for camera


### Getting started
- How to set up the environment Raspberry, camera and see if they are working...
...
- Install the latest raspbian OS availabile and follow these commands:
```bash
sudo apt update
sudo apt upgrade
```

- In order to check if the camera module is working type the following commands: 
```bash
libcamera-hello --qt-preview
```
If your camera is not responding make sure you have the right configuiration in the ```/boot/firmware/config.txt```.
